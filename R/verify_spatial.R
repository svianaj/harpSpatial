...
#' @param box_sizes Scales used for fuzzy methods like FSS. A vector of odd box sizes.
#' @param sqlite_path If specified, SQLite files are generated and written to
#'   this directory.
#' @param ... Arguments dependent on \code{file_format}, passed to to read_XXX_interpolate.
  ...
  ...)
{
  ...
  # Rather, we loop over all observation times and then over all forecasts valid for those times.
  ...
  #       - read accumulated fields only once (e.g. "acc3h = 6h - 3h" also re-uses the 3h field) 
  # Alterntive strategy: loop by fcdate, and cache all obs in a list by leadtime
  #     next fcdate -> "ldt -= by" ; drop negative ldt ; read missing obs
  # For an accumulated variable (precip), the minimum lead time is the accumulation time. Otherwise zero.
  ...
  if (missing(end_date)) {
    ...
    # add the last fc hour
    edate <- edate + (24 * 3600 / harpIO:::parse_by(by) - 1) * harpIO:::parse_by(by)
  }
  ...
  lt_scale <- switch(lt_unit, "h"=3600, "m"=60, "s"=1, stop("unknown lt_unit ", lt_unit))
  lead_time <- lead_time * lt_scale
  if (prm$accum > 0) {
    lead_time <- lead_time[which(lead_time >= prm$accum & lead_time %% prm$accum == 0)]
  }
  all_fc_dates <- seq(as.numeric(sdate), as.numeric(edate), harpIO:::parse_by(by)) %>% lubridate::as_datetime()
  ...
  read_fc <- get(paste0("read_", fc_file_format))
  read_ob <- get(paste0("read_", ob_file_format))

  get_ob <- function(obdate) {
    obfile <- get_filenames(file_date = format(obdate, "%Y%m%d%H"),
                            file_path = ob_file_path, file_template=ob_file_template,
                            parameter = parameter)
    message("reading", obfile)
    do.call(read_ob, c(list(filename=obfile, parameter = parameter), ob_options))
  }
  ...
  get_fc <- function(fcdate, lead_time) {
    ...
    message("reading", fcfile)
    do.call(read_fc, c(list(filename=fcfile, parameter = parameter, lead_time=lead_time),
                       fc_options))
  }
  ...
  for (ob in seq_along(all_ob_dates)) {  # (obdate in all_ob_dates) looses POSIXct class
    ...
    if (prm$accum > 0) { # an accumulated field like precipitation
      if (is.null(ob_accumulation) || ob_accumulation < 0) {
        ...
        # TODO:
        stop("Accumulated obs fields not yet implemented.")
      } else {
        ostep <- harpIO:::parse_by(ob_accumulation)
    ...
    if (!is.null(ob_interp_method)) {
      if (is.null(init$regrid_ob)) {
        ...
        init$regrid_ob <- meteogrid::regrid.init(olddomain = obfield, newdomain = verif_domain,
    ...
    for (fc in seq_along(valid_fc_dates)) {
      ...
      message("   +++ fcdate = ", format(fcdate,"%Y%m%d-%H%M"), " +++ ldt = ", ldt/lt_scale, lt_unit)

      fcfield <- get_fc(fcdate, ldt/lt_scale)
      if (inherits(fcfield, "try-error")) { # e.g. missing forecast run
        if (harpenv$verbose) message("..... Forecast not found. Skipping.", .immediate = TRUE)
        next
      }
      ...
      if (!is.null(fc_interp_method)) {
        if (is.null(init$regrid_fc)) {
          ...
          init$regrid_fc <- meteogrid::regrid.init(olddomain = fcfield, newdomain = verif_domain,
